...
export default function OneAdV2Page({
  ...
  const ImageCol = () => (
    <Col md={24} lg={12}>
      <div>
        <AdSlide
          productName={data?.[`title_${locale}`]}
  ...
  const DetailsCol = () => (
    ...
    >
      <div style={{ margin: "0px 0" }}>
        <ProductNameText>{data?.[`title_${locale}`]}</ProductNameText>

        {data?.[`description_${locale}`].split(". ").map((x) => {
          return (
            <Text
              key={x}
              as="p"
        ...
        <Text
          ...
          color={"red"}
        >
          {Taddad("priceForOneItem")}: {data?.price} ${/* • {x}. */}
        </Text>
      </div>
