import { EditOutlined, CloseCircleOutlined, CheckOutlined, FontColorsOutlined } from "@ant-design/icons";
import { Button, Col, Row, Tooltip, Input, InputNumber, Select, Tag, message } from "antd";
...
const reqUrl = process.env.NEXT_PUBLIC_HOST + process.env.NEXT_PUBLIC_ALL_UPDATE_SETTINGS;
...
export default function SettingsPageContent(props) {
  ...
  const submitColor = useCallback(async () => {
    ...
    if (fontColor?.id) {
      try {
        const { data: res } = await axiosInstance.post(reqUrl, formData, { httpsAgent: httpsAgent }, configHeader);

        console.log("res :>> ", res);
        if (res.status === true) {
          message.success(
            t("common:messages.add", {
              name: router.locale === "ar" ? "Item" : router.locale === "en" ? "Item" : "Item",
            })
          );
        }
      } catch (error) {
  ...
  return (
    <Row gutter={[24, 24]} style={{alignItems: 'center', justifyContent: 'center', display: 'flex' }}>
      <Col span={24}>
        ...
        <div style={{ display: "flex", flexDirection: "row" }}>
          {<Text as="h1">Settings</Text> !== 0 && (
            <Select
              ...
              onChange={(e) => onChangeHeadingSelect({ value: e, id: fontFamilyArr[0].id })}
          ...
          <button style={{ width: 160, height: 30, margin: 6 }} onClick={() => submitFontFamily()}>
            submit Text font
          </button>
        </div>
      </Col>
      <Col span={24}>
        <div style={{ display: "flex", flexDirection: "row", justifyContent: "center", alignItems: "center" }}>
          ...
          <button style={{ width: 120, height: 30, margin: 6 }} onClick={() => submitColor()}>
            submit color
          </button>
        </div>
      </Col>
        ...
        <div style={{ display: "flex", flexDirection: "row" }}>
          {<Text as="h1">Settings</Text> !== 0 && (
            <Select
              ...
              placeholder={"choose color"}
              defaultValue={fontFamilyArr[0]}
              onChange={(e) => onChangeTextSelect({ value: e, id: fontFamilyArr[0].id })}
              options={fontFamilyArr.map((x) => {
                return { label: x.fontFamily, value: x.id };
              })}
              tagRender={tagRender}
